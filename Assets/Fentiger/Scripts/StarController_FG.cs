...
public class StarController_FG : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(transform.position + Vector3.up * 2f, Vector3.down, out RaycastHit hit, 3f, LayerMask.GetMask("Player")) && hit.transform.GetComponent<UIManager_FG>().AddXP(25))
        {
            ...
            StartCoroutine(GAMEMANAGER.Instance.GetComponent<LedsController>().SingleBlink(hit.transform.name == "Player1", "BLUE", GameObject.Find("GAMEMANAGER").GetComponent<Generator_FG>().initialMultiplayer));
            Destroy(gameObject);
        }
    }
}
